...
#if defined(NDEBUG)
#error "Trepco cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in utrep) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Trepco Signed Message:\n";
...
bool ContextualCheckCoinSpend(const CoinSpend& spend, CBlockIndex* pindex, const uint256& txid)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
            return error("%s : zTrep with serial %s is already in the block %d\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTrep spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckCoinSpend(spend, chainActive.Tip(), txid))
                    return state.Invalid(error("%s: zTREP spend in tx %s failed to pass context checks", __func__, txid.GetHex()));
            }
        } else {
            LOCK(pool.cs);
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        ...
        nSubsidy = 1 * COIN;
    } else if (nHeight > 200 && nHeight <= 65300) {
        nSubsidy = 280 * COIN;
    } else if (nHeight > 65301 && nHeight <= 130100) {
        nSubsidy = 252 * COIN;
    } else if (nHeight > 130101 && nHeight <= 194900) {
        nSubsidy = 226.8 * COIN;
    } else if (nHeight > 194901 && nHeight <= 259700) {
        nSubsidy = 204 * COIN;
    } else if (nHeight > 259701 && nHeight <= 324500) {
        nSubsidy = 183.6 * COIN;
    } else if (nHeight > 324501 && nHeight <= 389300) {
        nSubsidy = 165.2 * COIN;
    } else if (nHeight > 389301 && nHeight <= 454100) {
        nSubsidy = 149 * COIN;
    } else if (nHeight > 454101 && nHeight <= 518900) {
        nSubsidy = 134 * COIN;
    } else if (nHeight > 518901 && nHeight <= 583700) {
        nSubsidy = 120.4 * COIN;
    } else if (nHeight > 583701 && nHeight <= 648500) {
        nSubsidy = 108.4 * COIN;
    } else {
        nSubsidy = 2 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Trepco
...
{
    RenameThread("trepco-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZTREPMinted()
{
...
void RecalculateZTREPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTREP supply
...
bool RecalculateTREPSupply(int nHeightStart)
{
...
bool UpdateZTREPSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (Params().Zerocoin_Block_RecalculateAccumulators() >= 0 && pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateTREPSupply(Params().Zerocoin_StartHeight());
    }

    //Track zTREP money supply in the block index
    if (!UpdateZTREPSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zTREP supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zTrepSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TREP to zTREP
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTrep spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTrep serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTrep mints and %d zTrep spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Trepco: We use certain sporks during IBD, so check to see if they are
