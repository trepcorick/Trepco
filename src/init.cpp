...
#include "config/trepco-config.h"
#endif
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "trepco.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "trepcod.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the TREP and zTREP money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in TREP/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in TREP/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, trepco, (obfuscation, swifttx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in TREP/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Trepco specific functionality (Masternodes, Zerocoin, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    // strUsage += HelpMessageOpt("-backupztrep=<n>", strprintf(_("Enable automatic wallet backups triggered after each zTrep minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizetrepcoamount=<n>     " + strprintf(_("Keep N TREP anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Trepco Core Developers"), "2018")) + "\n" +
           "\n" + 
...
/** Sanity checks
 *  Ensure that Trepco is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Trepco Core is shutting down."));
...
#endif
    // Make sure only a single Trepco process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Trepco Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Trepco specific: zerocoin and spork DB's
                ...
                // Trepco: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZTREPMinted();
                        RecalculateZTREPSpent();
                    }
                    RecalculateTREPSupply(1);
                }
                ...
                // Trepco: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Trepco Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Trepco Core to complete") << "\n";
        ...
        bool fEnableZTrepBackups = GetBoolArg("-backupztrep", true);
        pwalletMain->setZTrepAutoBackups(fEnableZTrepBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeTrepcoAmount = GetArg("-anonymizetrepcoamount", 0);
//    if (nAnonymizeTrepcoAmount > 999999) nAnonymizeTrepcoAmount = 999999;
//    if (nAnonymizeTrepcoAmount < 2) nAnonymizeTrepcoAmount = 2;
    ...
    LogPrintf("Anonymize Trepco Amount %d\n", nAnonymizeTrepcoAmount);
    ...
    /* Denominations
       ...
       For example:
       1TREP+1000 == (.1TREP+100)*10
       10TREP+10000 == (1TREP+1000)*10
    */
