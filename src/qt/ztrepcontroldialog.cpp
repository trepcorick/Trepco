...
#include "ui_ztrepcontroldialog.h"
...
std::list<std::string> ZTrepControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZTrepControlDialog::listMints;

ZTrepControlDialog::ZTrepControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZTrepControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZTrepControlDialog::~ZTrepControlDialog()
{
    delete ui;
}

void ZTrepControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZTrepControlDialog::updateList()
{
...
void ZTrepControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZTrepControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZTrep_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZTrepControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZTrepControlDialog::GetSelectedMints()
{
...
void ZTrepControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
