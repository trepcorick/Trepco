...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::TREP);
    ...
    if (!settings.contains("nAnonymizeTrepcoAmount"))
        settings.setValue("nAnonymizeTrepcoAmount", 1000);

    nAnonymizeTrepcoAmount = settings.value("nAnonymizeTrepcoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeTrepcoAmount"))
        SoftSetArg("-anonymizetrepcoamount", settings.value("nAnonymizeTrepcoAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in trepco.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTrepcoAmount:
            return QVariant(nAnonymizeTrepcoAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeTrepcoAmount:
            nAnonymizeTrepcoAmount = value.toInt();
            settings.setValue("nAnonymizeTrepcoAmount", nAnonymizeTrepcoAmount);
            emit anonymizeTrepcoAmountChanged(nAnonymizeTrepcoAmount);
            break;
        case CoinControlFeatures:
